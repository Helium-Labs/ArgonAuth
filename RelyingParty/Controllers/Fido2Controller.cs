...
public class FidoController : Controller
{
    ...
    )
    {
        try
        {
            ...
            // Check dwt.hash and options.Challenge are equal values
            ...
            // Meaning the server generated the DWT claims, except for the session key, which is generated by the client.
            // Passed verification, so we can now sign the DWT with the server account.
            var rpSignature = CryptoHelper.Ed25519SignWithMasterKey(dwt.hash);
            dwt.FillRpSign(rpSignature);
