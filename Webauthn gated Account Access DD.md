...
4. **Definitions**:
    - `PK_sess`: 32-byte public key from the `sess` X25519 keypair created client-side, injected into the Webauthn (DIDT) challenge during the assertion ceremony.
...
As strong as the Webauthn flow, where the only way to compromise the security is through a sophisticated phishing or MITM incident where the
