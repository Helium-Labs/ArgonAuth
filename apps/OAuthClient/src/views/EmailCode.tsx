...
const EmailCode = (): JSX.Element => {
  ...
  const handleVerifyWork = async (): Promise<void> => {
    ...
    try {
      await keychain.getEmailCodeIsValid(username, emailCode)
      toast.success('Code is valid')
      await keychain.register({
        username,
        redirectUri,
        state,
        codeChallenge,
  ...
  return (
    <main className="h-screen flex flex-col justify-center items-center">
      <div className="mx-auto max-w-lg text-center flex items-center space-x-2 my-4">
        <h1 className="text-2xl font-bold sm:text-3xl">Email Code</h1>
        <svg
      ...
      <div className="flex space-x-4">
        {code.map((num, index) => (
          <input
            ...
            className="w-12 h-12 rounded-lg border-2 border-gray-300 text-center text-xl font-medium focus:border-blue-500 focus:outline-none"
            pattern="\d*"
            inputMode="numeric"
            autoFocus={index === 0}
          />
      ...
      <button
        ...
        className="inline-block rounded-lg bg-blue-500 px-5 py-3 my-5 text-sm font-medium text-white flex transition-all hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
      >
        {isLoading && <Spinner />}
        Verify
      </button>
