...
export default class Keychain {
  ...
  public async signIn (username: string): Promise<string> {
    ...
    const signInOptions = await Fido2Client.fidoAssertionOptionsPost({ username, userVerification: 'Required' }, cspkB64)
    ...
    const response = await Fido2Client.fidoMakeAssertion(makeAssertionRequest)
    const authResponse: AssertionResponseModel = response.data
    AssertDefined(authResponse.verifyAssertionResult, 'authResponse.verifyAssertionResult must be defined')
  ...
  public async exchangeCodeForJWT (exchangeCode: string): Promise<string> {
    ...
    const exchangeCodeResponse = await Fido2Client.fidoExchangeCodeForJwt(body)
  ...
  public async getEmailCodeIsValid (username: string, emailCode: string): Promise<boolean> {
    const response = await Fido2Client.fidoVerifyEmailCode({ email: username, code: emailCode })
  ...
  public async initRegister (username: string): Promise<void> {
    ...
    const options = await Fido2Client.fidoMakeCredentialOptions(
      {
        username,
  ...
  public async jwtIsValid (jwt: string): Promise<boolean> {
    const response = await Fido2Client.fidoTokenIsValid(jwt)
    return response.data
  }
  ...
  public async register (username: string, emailCode: string): Promise<string> {
    ...
    const registerResponse = await Fido2Client.fidoMakeCredential(makeCredentialsRequest)
